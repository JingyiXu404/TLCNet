...
def arugment(img,gt, hflip=True, rot=True):
    ...
    return img, gt
# def arugment_3(img,gt,lq, hflip=True, rot=True):
#     hflip = hflip and random.random() < 0.5
#     vflip = rot and random.random() < 0.5
#
#     if hflip:
...
class MSRDataset(data.Dataset):
    def __init__(self, root1, root2,root3, patch_size, normalization=False, transform=False, type = 'train'):
        self.root1 = root1  # LR target
        self.root2 = root2  # HR guide
        self.root3 = root3  # HR target
        self.file1_lists = []
        self.file2_lists = []
        self.file3_lists = []
        ...
        for dir in np.array([x.path for x in os.scandir(self.root3)]):
            files = np.array([x.path.decode('utf-8') for x in os.scandir(dir)])
            self.file3_lists = np.append(self.file3_lists, files)
        self.file1_lists.sort()
        self.file2_lists.sort()
        self.file3_lists.sort()
        self.image1_files = self.file1_lists
        self.image2_files = self.file2_lists
        self.image3_files = self.file3_lists
    ...
    def __getitem__(self, index):
        ...
        img3_path = self.image3_files[index]
        if self.transform:
            img1 = self.train_transform(Image.open(self.image1_files[index]))
            img1 = np.array(img1)
            img2 = self.train_transform(Image.open(self.image2_files[index]))
            img2 = np.array(img2)
            img3 = self.train_transform(Image.open(self.image3_files[index]))
            img3 = np.array(img3)
        else:
            img1 = imread_uint(self.image1_files[index],n_channels=1)
            img2 = imread_uint(self.image2_files[index],n_channels=1)
            img3 = imread_uint(self.image3_files[index],n_channels=1)
        ...
        if self.type == 'train':
            ...
            patch3 = img3[rnd_h:rnd_h + self.patch_size, rnd_w:rnd_w + self.patch_size, :]
        else: #type == 'test'
            patch1 = img1
            patch2 = img2
            patch3 = img3
        ...
        patch3 = torch.from_numpy(np.ascontiguousarray(np.array(patch3))).permute(2, 0, 1).float()/255.

        # a = tensor2uint(patch1.detach().float())
        # b = tensor2uint(patch2.detach().float())
        # c = tensor2uint(patch3.detach().float())
        # cv2.imwrite('patch1.png', a)
        # cv2.imwrite('patch2.png', b)
        # cv2.imwrite('patch3.png', c)
        return patch1, patch2, patch3 #target, guide, gt
...
class Dataset_CIDIS_c3_nogt(data.Dataset):
    def __init__(self,dataset,path,patch=64,scale=4,train=True):
        super(Dataset_CIDIS_c3_nogt, self).__init__()
        self.scale = scale
        self.patch = patch
        self.train = train
        ...
        self.transform = transforms.Compose([transforms.ToTensor()])
        if dataset=='CIDIS':
            LQ_dir = os.path.join(path, "thermal")
    ...
    def __getitem__(self, idx):
        Guide_data = self.load_color(self.Guide_list[idx])
        LQ_data = self.load_thermal(self.LQ_list[idx])
        return self.totensor(LQ_data), self.totensor(Guide_data), self.totensor(LQ_data)
class Dataset_CIDIS_c3_PBVS(data.Dataset):
    def __init__(self,dataset,path,patch=64,scale=16,train=True):
        super(Dataset_CIDIS_c3_PBVS, self).__init__()
        self.scale = scale
        self.patch = patch
        self.train = train
        self.dataset = dataset
        ...
        if dataset=='CIDIS':
            if train:
                HQ_dir = os.path.join(path, "thermal/GT")
            else:
                HQ_dir = os.path.join(path, "thermal/GT_x"+str(scale))

            Guide_dir = os.path.join(path, "visible")
            LQ_dir = os.path.join(path, "thermal/LR_x"+str(scale))
        files = os.listdir(HQ_dir)
        files.sort()
        self.HQ_list = [os.path.join(HQ_dir, file) for file in files]
        files = os.listdir(Guide_dir)
...
class Dataset_CIDIS_c3(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # cv2.imwrite('patch3.png', Guide_data * 255)
        return self.totensor(bicubic), self.totensor(Guide_data), self.totensor(HQ_data)
class Dataset_CIDIS(data.Dataset):
    def __init__(self,dataset,path,patch=64,scale=4,train=True):
        super(Dataset_CIDIS, self).__init__()
        self.scale = scale
        self.patch = patch
        self.train = train
        self.dataset = dataset
...
class MDNDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # a = tensor2uint(patch1.detach().float())
        # b = tensor2uint(patch2.detach().float())
        # c = tensor2uint(patch1_gt.detach().float())
        # cv2.imwrite('patch1.png', a)
        # cv2.imwrite('patch2.png', b)
        # cv2.imwrite('patch3.png', c)
...
def save_tensor_channels(tensor, output_dir='/mnt/hdd1/XJY/GDSR_consistency/Pansharpening/',name='gt'):
...
if __name__ =="__main__":
    root_path_train = '/mnt/ssd1/XJY/GDSR/nyu_data/'
    root_path_train2 = '/mnt/ssd1/XJY/GDSR/RGBT/CIDIS_200/train/'
    root_path_test = '/mnt/ssd1/XJY/GDSR/NYUV2/test/'
    root_path_test2 = '/mnt/ssd1/XJY/GDSR/RGBT/CIDIS_200/test/'
    root_path_test_Middle = '/mnt/ssd1/XJY/GDSR/Middle/'
    root_path_test_Lu = '/mnt/ssd1/XJY/GDSR/Lu/'
    root_path_test_RGBDD = '/mnt/ssd1/XJY/GDSR/RGB-D-D/'
    patch_size = 128
    scale = 4
    dr_dataset_train = Dataset_CIDIS(dataset='CIDIS_200_enhance',path=root_path_train2, patch=patch_size, scale=8)
    loader_train = DataLoader(dr_dataset_train, batch_size=1, num_workers=0, shuffle=True)
    dr_dataset_test = Dataset_CIDIS(dataset='CIDIS_200_enhance',path=root_path_test2, patch=patch_size, scale=8, train=False)
