...
class TLCNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, num_of_layers=5,nc_x=[32, 64, 128],nb=2,d_size=3):
        ...
        for i in range(num_of_layers):
            self.hypa_list.append(HyPaNet(in_nc=in_channels, out_nc=6*in_channels))
    ...
    def forward(self, x,y):
        ...
        for i in range(self.num_of_layers):
            ...
            lambda1 = hypas[:, 0:self.in_channels].unsqueeze(-1)
            lambda2 = hypas[:, self.in_channels:self.in_channels*2].unsqueeze(-1)
            eta1 = hypas[:, self.in_channels*2:self.in_channels*3].unsqueeze(-1)
            eta2 = hypas[:, self.in_channels*3:self.in_channels*4].unsqueeze(-1)
            eta3 = hypas[:, self.in_channels*4:self.in_channels*5].unsqueeze(-1)
            gamma = hypas[:, self.in_channels*5:self.in_channels*6].unsqueeze(-1)
            z, m, n, b, v, u, w, H = self.update[i](x,y,z,m,n,b,v,u,w,lambda1, lambda2, eta1,eta2,eta3, gamma)
            preds.append(torch.mean(z, dim=1, keepdim=True))
            # preds.append(z)
...
if __name__ == '__main__':
    ...
    a_out = net(xy)
    # print(a_out.shape)
    macs, params = get_model_complexity_info(net, (2, 128,128), as_strings=True,print_per_layer_stat=True, verbose=True)
